...
1. **네트워크를 절대 사용하지 않습니다.** 클라우드 호출, 원격 분석, 크래시 업로드,
   원격 모델 다운로드를 금지합니다.
2. **모든 AI/ML은 기기에서 실행됩니다.** 경량 분류기는 앱에 포함하고, 선택형 대형 LLM은
   사용자가 로컬 저장소에서 직접 가져오는 경우에만 사용합니다.
...
- Retrofit, OkHttp, Ktor client, Volley, Apollo, Firebase, 원격 분석 및 크래시 업로드 SDK를
  금지합니다.
...
- 로컬 백업 v5에는 규칙 모드와 지원 조건 트리, 이름 있는 프로필, 채널·앱·시간대·의미별
  일별 요약, 선택한 설정을 포함하며 v1~v4도 계속 복원합니다. 패키지별 7종 의미 학습 투표는
...
:data            Room v12, DataStore, 저장소 구현, 백업, 매퍼
...
- 번들 분류기의 테스트는 고정 모델과 픽스처로 정확한 라벨을 검증합니다.
- 선택형 LLM은 사용자 가져오기, 기기 조건 확인, 결정적 테스트 더블을 사용합니다.
- 학습은 사용자의 명시적 피드백을 로컬 가중치/집계에 반영하는 점진적 방식입니다.
- 훈련 데이터나 기울기를 내보내지 않습니다.
...
- 관찰 규칙은 켜진 로컬 LLM 신호를 동작 없이 사용할 수 있지만 활성 규칙이 자동 동작에
  사용하려면 별도 LLM 자동 동작 설정이 켜져 있어야 합니다.
...
- 처리 설명은 lane, 조건 종류, 3상태 결과, 깊이, 순서만 최대 128노드로 저장합니다. 트레이스
  행에는 비교값, 알림 내용, LLM reasoning을 저장하지 않습니다.
...
- 오프라인 JVM 테스트와 Gradle 게이트가 Debug/Release의 `INTERNET` 권한과 네트워크
  의존성을 검사합니다.
- Room 마이그레이션은 `:data/src/androidTest`의 `MigrationTestHelper`로 v3→v12 순차
  업그레이드, 기존 데이터 보존, 이진 광고 피드백의 7종 의미 이관을 검증합니다.
