...
- **온디바이스 LLM 엔진은 MediaPipe Tasks GenAI**
  (`com.google.mediapipe:tasks-genai`)와 `LlmInference`입니다. 사용자가 준비한 호환되는
  self-contained 양자화 text 모델은 APK에 포함하지 않으며 Storage Access Framework로
  직접 가져온 뒤 앱의 `filesDir`에 저장합니다. 앱은 모델을 다운로드하지 않고,
  AlarmControl 프로젝트도 LLM이나 모델 학습 파이프라인을 배포하지 않습니다. 모델이
  없거나 로드에 실패하면 `OnDeviceLlmManager`가 `Unavailable`을 보고하고 규칙 + TFLite
  경로로 돌아갑니다.
...
- 생성형 LLM은 AlarmControl이 포함하거나 제공하지 않습니다. 사용자가 호환되는 로컬 파일을
  선택하면 앱 전용 `filesDir`에 원자적으로 복사합니다. 가져올 때 SHA-256 사이드카를 함께
  원자적으로 기록하고 이후 초기화마다 전체 파일을 해시해 누락되거나 불일치한 무결성 기록을
  거부합니다. 이는 가져온 뒤 파일 변경을 탐지하지만 모델 제작자를 인증하지는 않습니다.
  누락·손상·변경된 모델은 정상적으로 성능 저하 처리합니다.
...
- 알림 상세 내용 저장은 안전한 최초 초기화 후 기본값이 켜지며 기본 보존 기간은 7일입니다.
  사용자는 1일, 3일, 7일, 14일, 30일 중에서 선택하거나 앱별 제외 또는 저장 끄기를 할 수
  있습니다. `SECRET`이 아닌 제목·본문만 길이를 제한해 Android Keystore AES-256-GCM 키로
  암호화하고 별도 자식 행에 저장합니다. 목록·검색·통계·로그·백업은 평문을 읽거나 포함하지
  않습니다. 설정을 끄면 모든 암호문과 내보낼 수 없는 키를 즉시 삭제합니다.
...
- 번들 7종 의미 encoder는 기본값이 켜짐이지만 사용자가 끌 수 있습니다. 끄면 모델을 호출하지
  않고 의미 조건에 의존하는 동작은 fail-open 처리하며 관련 없는 규칙 신호는 계속 사용합니다.
...
- 인사이트용 이벤트 행에는 내용 없는 최소 메타데이터만 저장합니다. 제목·본문은 별도의
  Android Keystore 암호문 자식 행에 사용자가 선택한 1~30일 동안만 보관하며 목록 쿼리와
  통계에서 읽지 않습니다.
